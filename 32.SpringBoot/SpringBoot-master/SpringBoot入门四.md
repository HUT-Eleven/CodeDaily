...
Springboot默认的缓存是ConcurrentMapCacheManager==ConcurrentMapCache,将数据保存在ConcurrentsMap<Object,Object>.

实际生产中一般会缓存中间件，比如redis、memcached、ehcache。

#### 1、导入依赖

```xml
进入springboo官网，redis starts
```

#### 2、修改配置文件

```properties
spring.redis.host=192.168.179.131（注意：是host，不是spring.redis.url）
spring.redis.port=6379
```

#### 3、Redis的Template

> 操作redis，可以用原生的Jedis操作，也可以用SpringBoot提供的两个redis模板来操作。

**StringRedisTemplate**  ：操作 k-v都是string的

**RedisTemplate** ：操作 k-v都是对象的（这个肯定是可以替代StringRedisTemplate，因为频率航操作String比较多，所以单独补充了StringRedisTemplate）

两者方法一样，只是根据不同的数据类型需求，选择其一。

大致的操作也分为这5种，以StringRedisTemplate为例

```
stringRedisTemplate.opsForValue()  --String（就String对应的方法是value结尾的）
...
> 一般我们会想保存在redis中的数据时json格式的，这样比较直观。方法有两种
> 1.使用Gson、FastJson、JsonUtils这些工具，把对象转为json字符串
>
> 2.自定义序列化器
...
```java
//搜索RedisAutoConfiguration-->最底下有个内部类RedisConfiguration，里面由两个方法就是生成RedisTemplate和StringRedisTemplate的，复制生成RedisTemplate的方法到MyRdisConfig，把值的Object改成对应的javabean生成相对应javaBean的template。
@Configuration
...
**整合redis集群和连接池**:https://blog.csdn.net/qq_31256487/article/details/83144088
